...
export default {
  ...
  methods: {
    addGame() {
      if (this.alias == null) {
        this.feedback =
          "Please singup or singin before adding a game (guest can not create a new game)";
        window.alert(
          "Please singup or singin before adding a game (guest can not create a new game"
        );
        return;
      ...
      refGames.get().then(doc => {
        if (doc.exists) {
          ...
          while (tableBoard.length != 25) {
            // exectaly 25 words to board game
          ...
          while (black_list_two.length != 4) {
            randomCell = Math.floor(Math.random() * tableBoard.length);
            if (
              !black_list_two.includes(tableBoard[randomCell]) &&
          ...
          for (i = 0; i < 25; i++) {
            ...
            if (black_list_one.includes(tableBoard[i])) {
              // not exceed from black list
          ...
          this.$router.push({
            name: "Game",
            params: {
              player_one_alias: this.alias,
              turn: this.alias,
              conected_player: this.alias
            }
          });
        }
      });
