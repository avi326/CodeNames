      ...
      if (this.defineWord) {
        ...
        list_of_words.forEach(main_word => {
          list_of_words.forEach(word => {
            db.collection("common_words")
              .doc(main_word)
              .set(
                {
                  common_words_array: firebase.firestore.FieldValue.arrayUnion(
                    word
                  )
                },
                { merge: true }
            ...
            db.collection("common_words")
              ...
              .set(
                {
                  common_words_array: firebase.firestore.FieldValue.arrayRemove(
                    main_word
                  )
                },
                { merge: true }
        ...
        db.collection("word_data")
          .doc(this.defineWord)
          .set(
            {
              groups_array: firebase.firestore.FieldValue.arrayUnion(
                list_of_words_to_obj
              )
            },
            { merge: true }
          )
