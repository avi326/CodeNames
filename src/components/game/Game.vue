      ...
      <b-row>
        <b-col v-if="player_one && player_two">
          <GameMoves
            ...
            :startTurn="turn"
            :ref_db="ref"
            :conected_player="conected_player"
          ...
          <div class="mapTable">
            ...
            <table style="width:15%" class="mapTable table-bordered">
              <tr v-for="i in Math.ceil(map_player.length / 5)" :key="i">
                <td
                  v-for="cell in map_player.slice((i - 1) * 5, i * 5)"
                  :key="cell.id"
                  :class="cell"
                ></td>
              </tr>
...
export default {
  ...
  beforeDestroy() {
    // save the data of the game. player_one save because we need it once.
    if (this.conected_player==this.player_one) {
        this.save_game_data()
    }
  ...
  methods: {
    ...
    getNumOfMoves() {
      var ref = this.ref.onSnapshot(doc => {
        if (doc.exists) {
      ...
      if (this.count_moves == 12) {
        ...
        this.ref.get().then(doc => {
          if (!doc.exists) {
            ...
            if (
              blue_words_player_one.length == 0 &&
              blue_words_player_two.length == 0
            ) {
    ...
    end_game() {
      ...
      this.$dialog
        .confirm("אתה מעוניין לסיים את המשחק?")
        .then(function() {
          self.ref.update({
            num_of_moves: 12
          });
        ...
        .then(doc => {
            ...
            console.log('No such document!');
                ...
                var game_data = doc.data()
                db.collection("all_games_data").add(game_data)
                .then(function(docRef) {
                    console.log("add game_ data ID: ", docRef.id);
                })
                .catch(function(error) {
