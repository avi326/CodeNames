    ...
    <div>
      <div>
        <span v-if="rival_define">
          ...
          <h3
            style="background:PowderBlue; box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);"
          >{{ rival_define }} - {{ num_of_word_to_guess }}</h3>
        </span>
        <div>
          <ul class="show_words_right_guess" v-chat-scroll>
        ...
        <form class="card-panel" @submit.prevent="sendGuess">
          ...
          <div class="field">
            ...
            <multiselect
              ...
              :selected="value"
              deselect-label="חייב לבחור מילה"
              placeholder="חפש או בחר מילה"
              @update="updateValue"
            ></multiselect>
          </div>
          <span v-if="value">
            <label class="ask_guess">
              בטוח שהמילה
              <b>{{ value.name }}</b> מתאימה להגדרה
              <b>{{ rival_define }}</b>?
            </label>
          </span>
...
export default {
  name: "GuessDefine",
  props: { table_board: String, ref_db: Object },
  components: {
    Multiselect,
    FixGuess
  },
  ...
  methods: {
    ...
    sendGuess() {
      ...
      if (this.words_to_guess.includes(this.value.name)) {
        ...
        ref_moves.update({
          words_right_guess: firebase.firestore.FieldValue.arrayUnion(
            this.value.name
          )
        });
        ...
        ref_moves.update({
          words_worng_guess: firebase.firestore.FieldValue.arrayUnion(
            this.value.name
          ),
        ...
        var getDoc = this.ref_db.get().then(doc => {
          if (!doc.exists) {
            ...
            if (temp_arr.includes(this.value.name)) {
              ...
              this.$dialog
                .alert("נבחרה מילה שחורה של היריב. המשחק הסתיים!  ")
                .then(dialog => {
                  console.log("Closed");
                });
        ...
        .then(doc => {
          if (!doc.exists) {
            ...
            if (this.alias == player_two_alias) {
              ...
              this.ref_db.update({
                blue_words_player_one: firebase.firestore.FieldValue.arrayRemove(
                  value.name
                )
              ...
              this.ref_db.update({
                blue_words_player_two: firebase.firestore.FieldValue.arrayRemove(
                  value.name
                )
              });
            }
