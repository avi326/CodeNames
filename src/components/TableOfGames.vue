  ...
  <div id="TableOfGames">
    <b-container>
      ...
      <b-row class="justify-content-md-center">
        <b-col md="auto">
          <p v-if="alias" style="text-align: right;  direction: rtl;">
            ברוך הבא
            <b>{{ alias }}</b>, אתה יכול להתחיל לשחק!
          </p>
          <p v-else>
            שלום אורח, בבקשה
            <router-link :to="{ name: 'Signup'}">הירשם</router-link>או
      ...
      <b-row class="justify-content-md-center">
        <b-col md="auto">
          <p style="text-align: right;  direction: rtl;">
            אם יש מישהו שאתה רוצה לשחק איתו - תחפש אותו ברשימה ולחץ
            <b>הצטרף למשחק</b>.
            <br />אחרת, לחץ למטה על
            <b>משחק חדש</b> ותחכה שהוא יצטרף אליך.
            <br />אם אין מישהו שקבעת איתו. פתח
            <b>משחק חדש</b> ותחכה שיצטרפו אליך.
          </p>
          <table v-if="alias" style="text-align: right;  direction: rtl;">
            ...
            <tbody>
              <tr v-for="game in games" :key="game.id">
                <td>{{ game.player1 }}</td>
                <td v-if="game.player2==''">
                  <router-link
                    :to="{ name: 'Game', params: {player_one_alias: game.player1 , player_two_alias: alias , conected_player: alias}}"
                  >הצטרף למשחק</router-link>
                </td>
                <td v-else>{{ game.player2 }}</td>
                <td class="play_now">{{ game.countPlayers }}</td>
                <td v-if="alias == game.player1">
  ...
  methods: {
    deleteGame(id) {
      ...
      db.collection("games")
        .doc(this.alias)
        .delete();
      db.collection("table_of_players")
        .doc(id)
        .delete()
        .then(() => {
